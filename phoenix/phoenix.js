...
// Move window to NW, NE, SE, SW or center corner
...
/* 
  Fill Bindings
  Resize and move window to selected size
*/
...
Key.on('b', CONTROL_SHIFT, function() {
  App.launch(BROWSER_APP).focus();
});

Key.on('s', CONTROL_SHIFT, function() {
  App.launch(SLACK_APP).focus();
});

Key.on('i', CONTROL_SHIFT, function() {
  App.launch(IOS_SIMULATOR).focus();
});
